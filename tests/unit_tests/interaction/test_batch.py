...
async def test_batch_single_iterable_returns_results_in_order():
    ...
    with patch("requestcompletion.interaction.batch.call", new=mock_call):
        ...
        results = await batch(node, inputs)
...
async def test_batch_multiple_iterables_passes_all_args():
    ...
    with patch("requestcompletion.interaction.batch.call", new=mock_call):
        ...
        results = await batch(node, inputs1, inputs2)
...
async def test_batch_with_exceptions_returned():
    ...
    with patch("requestcompletion.interaction.batch.call", new=mock_call):
        ...
        results = await batch(node, inputs, return_exceptions=True)
...
async def test_batch_with_exceptions_raised():
    ...
    with patch("requestcompletion.interaction.batch.call", new=mock_call):
        ...
        with pytest.raises(RuntimeError, match="Fail!"):
            await batch(node, inputs, return_exceptions=False)
