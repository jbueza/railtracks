...
from requestcompletion.pubsub.subscriber import stream_subscriber

# ================= START Subscriber class tests ============


@pytest.mark.asyncio
async def test_subscriber_stores_callback_and_name(sync_callback_container):
...
@pytest.mark.asyncio
async def test_subscriber_custom_name(sync_callback_container):
    _, callback = sync_callback_container
...
@pytest.mark.asyncio
async def test_subscriber_trigger_sync(sync_callback_container):
...
@pytest.mark.asyncio
async def test_subscriber_trigger_async(async_callback_container):
...
@pytest.mark.asyncio
async def test_basic_publisher_publish_sync(started_publisher, sync_callback_container):
    state, callback = sync_callback_container
...
async def test_basic_publisher_publish_async(
    started_publisher, async_callback_container
):
    state, callback = async_callback_container
    started_publisher.subscribe(callback)
...
@pytest.mark.asyncio
async def test_publisher_context_manager(async_publisher, sync_callback_container):
    state, callback = sync_callback_container
...
@pytest.mark.asyncio
async def test_publisher_multiple_subscribers(started_publisher, msg_list_container):
...
@pytest.mark.timeout(0.5)
@pytest.mark.asyncio
async def test_basic_publisher(started_publisher):
    _message = None
...
@pytest.mark.asyncio
async def test_unsubscribe_prevents_future_messages(started_publisher):
    received = []
...
@pytest.mark.asyncio
async def test_unsubscribe_missing_raises(started_publisher):
    with pytest.raises(KeyError):
        started_publisher.unsubscribe("nope")

@pytest.mark.asyncio
async def test_unsubscribe(started_publisher):
    _message = None
...
@pytest.mark.asyncio
async def test_bad_unsubscribe(started_publisher):
    with pytest.raises(KeyError):
...
@pytest.mark.asyncio
async def test_bad_subscribe(started_publisher):
...
@pytest.mark.asyncio
async def test_ordering_of_messages(async_publisher):
    received = []
...
@pytest.mark.asyncio
async def test_mixed_sync_async_callbacks(async_publisher):
    received = []
...
@pytest.mark.asyncio
async def test_callback_raises_does_not_block_others(started_publisher):
    got = []
...
async def test_blocking_publisher(started_publisher):
    _message = []
...
@pytest.mark.asyncio
async def test_multiple_subs_with_blocking(started_publisher):
    _message_1 = []
    _message_2 = []
...
# ================= START Publisher exception/robustness tests ============
@pytest.mark.asyncio
async def test_exception_thrower(started_publisher):
...
# ================ END Publisher exception/robustness tests ===============

# ================= START Publisher listener tests ============

@pytest.mark.asyncio
async def test_listener_resolves_on_match(async_publisher):
...
@pytest.mark.asyncio
async def test_listener_with_result_mapping(async_publisher):
...
@pytest.mark.asyncio
async def test_listener_raises_post_shutdown(started_publisher):
...
@pytest.mark.asyncio
async def test_listener_timeout_no_message(async_publisher):
    fut = async_publisher.listener(lambda x: x == "never")
    with pytest.raises(asyncio.TimeoutError):
        await asyncio.wait_for(fut, timeout=0.1)

@pytest.mark.asyncio
async def test_listener_simple(async_publisher):
    _message = None
...
@pytest.mark.asyncio
async def test_listener_many_messages(async_publisher):
...
@pytest.mark.asyncio
async def test_precheck_listener(async_publisher):
        hw_listener = async_publisher.listener(lambda x: x == "hello world")
...
@pytest.mark.asyncio
async def test_get_listener_after_shutdown(async_publisher):
    hw_listener = async_publisher.listener(lambda x: x == "hello world")
...
@pytest.mark.asyncio
async def test_is_running_and_shutdown(started_publisher):
...
@pytest.mark.asyncio
async def test_publish_after_shutdown_raises(started_publisher):
    await started_publisher.shutdown()
    with pytest.raises(RuntimeError):
...
async def test_rcpublisher_logging_sub(dummy_publisher):
    from requestcompletion.pubsub.messages import RequestCompletionMessage
...
async def test_stream_subscriber_handles_streaming_true(
    streaming_message, streamed_object
):
    results = []
...
@pytest.mark.asyncio
async def test_stream_subscriber_skips_non_streaming():
    class DummyMsg:
        pass
...
@pytest.mark.asyncio
async def test_stream_subscriber_callback_exception(streaming_message):
    def bad(val):
        raise Exception("fail!")  # coverage for error handling

    handler = stream_subscriber(bad)
    # Should not raise error here, just pass silently
    await handler(streaming_message)
...
@pytest.mark.asyncio
async def test_publish_with_many_subscribers(async_publisher):
    order = []
...
@pytest.mark.asyncio
async def test_subscribe_unsubscribe_multiple(started_publisher):
    got = []
    ids = []
    for i in range(3):
...
@pytest.mark.asyncio
async def test_subscriber_autonames_are_unique(started_publisher):
    def cb1(x):
        pass
...
@pytest.mark.asyncio
async def test_double_shutdown_ok(started_publisher):
    await started_publisher.shutdown()
    # Should not error
    await started_publisher.shutdown()
...
@pytest.mark.asyncio
async def test_subscriber_id_uniqueness(async_publisher):
    ids = []
    for i in range(5):
