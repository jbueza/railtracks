...
def test_add_same_object():
    ...
    with pytest.raises(AssertionError) as e:
        heap._update_heap(
            MockLinkedObject(
                identifier=message_object.identifier,
    ...
    with pytest.raises(AssertionError):
        heap._update_heap(
            MockLinkedObject(
                identifier=message_object.identifier,
                message="Hello world 2",
                stamp=Stamp(900, 0, "Init"),
                parent=message_object,
            )
        )


def test_level_old_reference():
    heap = Forest[MockLinkedObject]()
    ...
    # note we are still referencing that original object
    updated_obj_2 = MockLinkedObject(
        identifier=message_object.identifier,
        message="Hello world",
        stamp=Stamp(901, 0, "Init"),
        parent=linked_obj,
    ...
    with pytest.raises(AssertionError):
        heap._update_heap(updated_obj_2)
...
def test_full_data(example_structure):
    forest, data = example_structure
    full_data = forest.full_data()
