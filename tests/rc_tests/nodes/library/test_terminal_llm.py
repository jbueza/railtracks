...
async def test_terminal_llm_as_tool_correct_initialization(model, encoder_system_message, decoder_system_message):
    ...
    decoder_tool_params = {rc.llm.Parameter("bytes_input", "string", "The bytes you would like to decode")}
...
async def test_terminal_llm_missing_tool_details(model, encoder_system_message):
    ...
    with pytest.raises(RuntimeError, match="Tool parameters provided but no tool details provided."):
...
@pytest.mark.asyncio
async def test_terminal_llm_tool_duplicate_parameter_names(model, encoder_system_message):
