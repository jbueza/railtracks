...
class StructuredToolCallLLM(OutputLessToolCallLLM[str], ABC):
    ...
    def __init__(self, 
        ...
        system_structured = SystemMessage("You are a structured LLM that can convert the response into a structured output.")
        self.structured_resp_node = structured_llm(output_model, 
    ...
    async def return_output(self) -> BaseModel:
        ...
        try:
            return await call(self.structured_resp_node, message_history=MessageHistory([UserMessage(last_message.content)]))
