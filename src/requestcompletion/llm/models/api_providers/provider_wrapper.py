...
class ProviderLLMWrapper(LiteLLMWrapper):
    def __init__(self, model_name: str, **kwargs):
        ...
        if provider_info[1] != provider_name:
            raise ValueError(
                f"Invalid model name: {model_name}. Model name must be a part of {self.__class__.model_type()}'s model list."
            )
    ...
    def chat_with_tools(self, messages, tools, **kwargs):
        if not litellm.supports_function_calling(model=self._model_name):
            raise ValueError(
                f"Model {self._model_name} does not support function calling. Chat with tools is not supported."
            )
