...
async def search_flights():
    ...
    airlines = ["American Airlines", "Delta Airlines", "United Airlines", "Southwest Airlines", "British Airways", "Air France"]
    flights = []

    for i in range(150):  # Generate 50 flights
        dep_city = random.choice(cities)
        arr_city = random.choice([city for city in cities if city != dep_city])  # Ensure arrival city is different
        departure_time = datetime.now() + timedelta(days=random.randint(1, 30), hours=random.randint(0, 23))
        arrival_time = departure_time + timedelta(hours=random.randint(1, 6))  # Flight duration between 1-6 hours
        flight = {
            ...
            "price": round(random.uniform(100, 1000), 2),  # Price between $100 and $1000
        }
...
def filter_flights(flights: list, departure_city: str, arrival_city: str):
    filtered_flights = [flight for flight in flights if flight["departure_city"] == departure_city and flight["arrival_city"] == arrival_city]
    if not filtered_flights:
        raise ValueError(f"No outbound flights found from {departure_city} to {arrival_city}.")
...
async def pick_flight(flights: list, departure_city: str, arrival_city: str):
    contracts = asyncio.gather(rt.call(filter_flights, flights, departure_city, arrival_city),
...
TLLMNode = rt.agent_node(llm_model=rt.llm.OpenAILLM("gpt-4o"), system_message="You are excellent chooser of random items from a list. You never make mistakes and have a god like ability to accomplish tasks.")

@rt.function_node
async def planner_with_llm():
    cities = []
    available_cities = ["New York", "Chicago", "Los Angeles", "Houston"]
    while True:

        result = await rt.call(TLLMNode, user_input=f"Choose a Random city from {available_cities}")

        if result.text in available_cities:
            available_cities.remove(result.text)
