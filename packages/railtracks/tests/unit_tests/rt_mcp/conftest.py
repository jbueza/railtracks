...
@pytest.fixture
def mock_FastMCP():
    with patch("railtracks.rt_mcp.to_node.FastMCP") as p:
        yield p

@pytest.fixture
def mock_MCPTool():
    with patch("railtracks.rt_mcp.to_node.MCPTool") as p:
        yield p

@pytest.fixture
def mock_func_metadata():
    with patch("railtracks.rt_mcp.to_node.func_metadata") as p:
        yield p
